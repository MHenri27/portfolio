...
document.getElementById("toolList").innerHTML = allTools
...
(function () {
  ...
  const layers = STACK_LAYERS.filter(([key]) => PROJECTS.some((p) => p.stack && p.stack[key]));
  document.getElementById("matrixWrap").style.setProperty("--rows", layers.length);
  m.style.setProperty("--n", PROJECTS.length);
  document.getElementById("mxLabels").innerHTML =
    `<span class="m-row head"></span>` + layers.map(([, label]) => `<span class="m-row">${esc(label)}</span>`).join("");
  let h = PROJECTS.map((p, i) => `<span class="m-col" data-c="${i + 1}">${esc(oneLine(p.title))}</span>`).join("");
  layers.forEach(([key]) => {
...
document.getElementById("moreTools").innerHTML = MORE_TOOLS.map(
  ([label, tools]) =>
